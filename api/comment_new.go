...
func commentNew(commenterHex string, domain string, path string, parentHex string, markdown string, state string, creationDate time.Time) (string, error) {
	...
	statement := `
		INSERT INTO
		comments (comment_hex, domain, path, commenter_hex, parent_hex, markdown, html, creation_date, state)
		VALUES   (?,         ?,     ?,   ?,           ?,        ?,       ?,   ?,           ?   );
	`
	err = db.Exec(statement, commentHex, domain, path, commenterHex, parentHex, markdown, html, creationDate, state).Error
	if err != nil {
...
func commentNewHandler(w http.ResponseWriter, r *http.Request) {
	...
	if smtpConfigured {
		go emailNotificationNew(d, path, commenterHex, commentHex, *x.ParentHex, state, *x.Title)
	}
}
