...
func TestConfigMaxConnections(t *testing.T) {
	...
	os.Setenv("COMMENTO_MAX_IDLE_DB_CONNECTIONS", "100")
	if err := configParse(); err != nil {
		t.Errorf("unexpected error when MAX_IDLE_DB_CONNECTIONS=100: %v", err)
		return
	}

	os.Setenv("COMMENTO_MAX_IDLE_DB_CONNECTIONS", "text")
	if err := configParse(); err == nil {
		t.Errorf("expected error with MAX_IDLE_DB_CONNECTIONS=text not found")
		return
	}

	os.Setenv("COMMENTO_MAX_IDLE_DB_CONNECTIONS", "0")
	if err := configParse(); err == nil {
		t.Errorf("expected error with MAX_IDLE_DB_CONNECTIONS=0 not found")
		return
	}

	os.Setenv("COMMENTO_MAX_IDLE_DB_CONNECTIONS", "-1")
	if err := configParse(); err == nil {
		t.Errorf("expected error with MAX_IDLE_DB_CONNECTIONS=-1 not found")
		return
	}
}
