...
func emailNotificationReply(d domain, path string, title string, commenterHex string, commentHex string, parentHex string, state string) {
	...
	statement := `
		...
		WHERE comment_hex = ?;
	`
	row := db.Raw(statement, parentHex).Row()
...
func emailNotificationNew(d domain, path string, commenterHex string, commentHex string, parentHex string, state string, title string) {
	...
	if p.Title == "" {
		p.Title, err = pageTitleUpdate(d.Domain, path, title)
		if err != nil {
